...
**Core Gameplay Loop & State Synchronization (Recent)**
...
- Advance endpoint (controller-only) transitions: `round_intro → scoreboard → round_intro(next) → finished`, emitting `state_update` each step.
